...
interface FieldConfig { key: keyof SessionData; label: string; mask?: boolean; halfWidth?: boolean; }
...
const ACCOUNT_SECTIONS: Array<{ name: string; icon: React.ReactNode; fields: FieldConfig[]; color: string; gradient: string }> = [
  {
    ...
    fields: [
      { key: "accountType", label: "ACCOUNT_TYPE", halfWidth: true },
      { key: "salutation", label: "SALUTATION", halfWidth: true },
      { key: "firstName", label: "FIRST_NAME", halfWidth: true },
      { key: "middleName", label: "MIDDLE_NAME", halfWidth: true },
      { key: "lastName", label: "LAST_NAME", halfWidth: true },
      { key: "dob", label: "D.O.B", halfWidth: true },
      { key: "gender", label: "GENDER", halfWidth: true },
      { key: "maritalStatus", label: "MARITAL_STATUS", halfWidth: true },
      { key: "fatherSpouseName", label: "FATHER/SPOUSE" },
      { key: "nationality", label: "NATIONALITY", halfWidth: true },
      { key: "occupation", label: "OCCUPATION", halfWidth: true },
      { key: "annualIncome", label: "ANNUAL_INCOME", halfWidth: true },
      { key: "sourceOfFunds", label: "SOURCE_OF_FUNDS", halfWidth: true }
    ],
  },
  {
    name: "KYC Documents", color: "#00E5FF", gradient: "linear-gradient(135deg,#0284C7,#00E5FF)", icon: <Shield className="w-4 h-4" />,
    fields: [
      { key: "pan", label: "PAN_NUMBER", halfWidth: true },
      { key: "aadhaar", label: "AADHAAR_NO", mask: true, halfWidth: true },
      { key: "idType", label: "ID_TYPE", halfWidth: true },
      { key: "idNumber", label: "ID_NUMBER", halfWidth: true },
  ...
  },
  {
    ...
    fields: [{ key: "mobile", label: "MOBILE", mask: true, halfWidth: true }, { key: "email", label: "EMAIL", halfWidth: true }],
  },
  {
    name: "Address Details", color: "#00E5FF", gradient: "linear-gradient(135deg,#0284C7,#00E5FF)", icon: <MapPin className="w-4 h-4" />,
    fields: [
      { key: "addressLine1", label: "ADDRESS" },
      { key: "addressCity", label: "CITY", halfWidth: true },
      { key: "addressState", label: "STATE", halfWidth: true },
      { key: "addressPin", label: "PIN", halfWidth: true }
    ],
  },
  {
    name: "Correspondence Address", color: "#00E5FF", gradient: "linear-gradient(135deg,#0284C7,#00E5FF)", icon: <MapPin className="w-4 h-4" />,
    fields: [
      { key: "corrSameAddress", label: "SAME_AS_CURRENT" },
      { key: "corrAddress", label: "ADDRESS" },
      { key: "corrCity", label: "CITY", halfWidth: true },
      { key: "corrState", label: "STATE", halfWidth: true },
      { key: "corrPin", label: "PIN", halfWidth: true }
    ],
  },
  {
    name: "Nominee Details", color: "#00E5FF", gradient: "linear-gradient(135deg,#0284C7,#00E5FF)", icon: <Users className="w-4 h-4" />,
    fields: [{ key: "nomineeName", label: "NAME" }, { key: "nomineeRelation", label: "RELATION", halfWidth: true }, { key: "nomineeDob", label: "DOB", halfWidth: true }],
  },
  {
    name: "Services Required", color: "#00E5FF", gradient: "linear-gradient(135deg,#0284C7,#00E5FF)", icon: <CreditCard className="w-4 h-4" />,
    fields: [
      { key: "wantsATM", label: "ATM_CARD", halfWidth: true },
      { key: "wantsChequeBook", label: "CHEQUE", halfWidth: true },
      { key: "wantsMobileBanking", label: "MOBILE", halfWidth: true },
      { key: "wantsSMS", label: "SMS", halfWidth: true }
    ],
  },
  {
...
const FormVisualization = ({
  ...
  const renderField = (field: FieldConfig, accentColor = "#4F6EF7") => {
    ...
    return (
      <div key={field.key} style={{ marginBottom: 10, gridColumn: field.halfWidth ? "span 1" : "span 2" }}>
        <span className="neo-field-label" style={{ color: accentColor }}>{field.label}</span>
        <div className={cls} style={{ minHeight: 40, padding: "8px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{
          ...
          {showValid && (
            <div className="animate-success-glow" style={{ width: 18, height: 18, background: "linear-gradient(135deg,#10B981,#22C55E)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Check className="w-2.5 h-2.5 text-white" />
            </div>
          )}
          {showInvalid && (
            <div style={{ width: 18, height: 18, background: "linear-gradient(135deg,#EF4444,#F87171)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <AlertCircle className="w-2.5 h-2.5 text-white" />
            </div>
          )}
          {isFilled && !isActive && (
            <Check className="w-3 h-3" style={{ color: "#10B981", flexShrink: 0 }} />
          )}
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-lg animate-fade-in-up">
        ...
        >
          ...
          <div style={{
            ...
            {activeSection.fields.map(f => renderField(f, activeSection.color))}
          </div>
        </div>
